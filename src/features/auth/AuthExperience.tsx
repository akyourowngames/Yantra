...
export default function AuthExperience({
  ...
  const handleForgotPassword = async () => {
    ...
    if (!email) {
      setStatus({
        kind: 'error',
        message: 'Enter your email address first so Yantra knows where to send the recovery link.',
      });
      return;
    }
    ...
    try {
      ...
      setStatus({
        kind: 'error',
        message: error instanceof Error ? error.message : 'Yantra could not start password recovery right now.',
      });
    }
  };
      ...
      <header className="fixed left-0 top-0 z-40 w-full px-6 py-6 md:px-8">
        <div className="mx-auto flex max-w-[1600px] items-center justify-between">
          ...
          <div className="hidden items-center gap-3 md:flex">
            <span className="h-2 w-2 rounded-full bg-white shadow-[0_0_16px_rgba(255,255,255,0.55)]" />
